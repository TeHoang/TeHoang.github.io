...
Nhận thấy phần $ac + c$ chính là output $state_1$ của LCG nếu như $state_0 = c$. Vậy ta hoàn toàn có thể loại bỏ đi phần hằng số, để thu được dãy $x'$ có mối quan hệ $x'_{i+1} \equiv ax'_i \pmod{m}$. Ta dễ dàng nhận thấy $x'_i \equiv a^i x'_0 \pmod{m}$ tức $a^i x'_0 - x'_i \equiv 0 \pmod{m}$. Mục tiêu của ta sẽ là đi tìm $x_{40}$ sau đó lấy $18$ bit đầu tiên, đó chính là $ee_{40}$ mà ta cần tìm. Ta xét một lattice $L$ được tạo ra bởi basis $B$ (đồng thời ta cũng thực hiện LLL trên $B$) như sau:
